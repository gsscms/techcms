...
<link href="<%=path %>/web/style.css" type="text/css" rel="stylesheet"/>

<script src="<%=path %>/web/focus.js" type="text/javascript"></script>	
<title>无标题文档</title>
<style>
/* .banner{width:1000px; height:275px; background:url(<%=path %>/web/pic/banner.png) no-repeat; margin-top:15px;} */
.detail{color: #012BB1;}
...
.show .leftbtn{background:url(<%=path %>/web/pic/left_btn.png)no-repeat;}
.show .rightbtn{background:url(<%=path %>/web/pic/right_btn.png)no-repeat;}
.show .img1{background:url(<%=path %>/web/pic/pic14.png) no-repeat;margin-right:15px;margin-left:13px;}
.show .img2{background:url(<%=path %>/web/pic/pic15.png) no-repeat;margin-right:15px;}
.show .img3{background:url(<%=path %>/web/pic/pic16.png) no-repeat;margin-right:15px;}
.topLi {background:url(<%=path %>/web/pic/pic20.png) no-repeat;align:center}
.topLiSelect {background:url(<%=path %>/web/pic/pic19.png) no-repeat;align:center}
...
.contentDownImgL{position:absolute;left:0;top:0;background:url(<%=path %>/web/pic/pic17.png) no-repeat;width:98px;height:87px;}
...
jQuery(document).ready(function() {
   $("#displayId").val("");
	...
	$.ajax({ 
	    ...
	    success: function(data){
		  if(data!=null){
// 		      item.composition.substr(0,150)
			$("#topicP").append(data.content.substr(0,630)+"........."
				+"<a class='detail' href='<%=path%>/web/topicDetail.jsp?id="+data.id+"'>[详细]</a>");	
		  }
...
<!-- 				<li>3</li> -->
	...
	<div id="bd1lfimg">
		<div>
			<dl class="show">
				<dt>
					<img src="<%=path %>/web/pic/banner_1.png"/></dt>

			</dl>
			<dl>
				<dt>
					<img src="<%=path %>/web/pic/banner_2.png"/></dt>
...
<!-- 			<p>  微创针刀镜技术“微创针刀镜技术”是在中医经筋理论和经络水通道学说的指导下，在《灵枢》“九针”以及现代小针刀的基础上，融入现代医学的解剖学理论和内窥镜技术，所形成的一种微创、可视的手术（需局部麻醉）。其擅长治疗类风湿性关节炎和膝骨性关节。该技术较一般手术操作简单、创伤小、康复快、副作用小、症状改善明显，具有一定的优势。特地来找李政主任看病的。凭借专业专注，赢得了“泉州男性守护者”的称号。</p> -->
		...
		<div class="contentDown">
			...
			<p  id="topicP"><img src="<%=path %>/web/pic/pic18.png" align ="right"/>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</p>
